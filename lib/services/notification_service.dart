    ...
    try {
      ...
      for (final line in output.split('\n')) {
        ...
        if (line.contains('android.title=') && currentPkg.isNotEmpty) {
          ...
          items.add(NotificationItem(id: currentPkg + items.length, appName: _getAppName(currentPkg), title: title, content: '', time: DateTime.now(), type: NotificationType.other, packageName: currentPkg, isSelected: true));
        }
      }
    } catch (_) {}
